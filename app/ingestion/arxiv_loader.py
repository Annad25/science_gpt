...
async def fetch_arxiv_papers(
    ...
    Downloads the full PDF for each paper and extracts the body text.
    If PDF extraction fails for any paper, it falls back to using
    the abstract only (prefixed by the title).
    ...
    for query in queries:
        ...
        try:
            papers = await asyncio.to_thread(_search_arxiv, query, max_papers)
...
def _search_arxiv(query: str, max_results: int) -> list[dict[str, Any]]:
    ...
    For each paper, attempts to download the PDF to a temp directory and
    extract full text.  Falls back to abstract if extraction fails.
    """
